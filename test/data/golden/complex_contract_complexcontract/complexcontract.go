...
// ComplexEventEvent returns a packable event for ComplexEvent
func (er EventRegistry) ComplexEventEvent() *ComplexEventEventDecoder {
...
func (m *ComplexFunctionMethod) MustDecode(data []byte) ComplexFunctionResult {
	...
	result := ComplexFunctionResult{}
	offset := 0
	if len(data) < offset+32 {
		panic("insufficient data for return value 0")
	}
	result.Success = decodeBool(data[offset : offset+32])
	offset += 32
...
func (e *ComplexEventEventDecoder) MustDecode(data []byte) ComplexEventEvent {
	...
	result := ComplexEventEvent{}
	offset := 0
	var nextOffset int
	result.Data, nextOffset = decodeBytes(data, offset)
	offset = nextOffset
	return result
}
...
func (e *ComplexErrorErrorDecoder) MustDecode(data []byte) ComplexErrorError {
	...
	result := ComplexErrorError{}
	offset := 0
	var nextOffset int
	result.Reason, nextOffset = decodeString(errorData, offset)
	offset = nextOffset
