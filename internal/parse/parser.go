...
func parseContract(sourceFile, contractName string, result types.ContractResult) (*types.Contract, error) {
	...
	// Parse methods
	methods, err := parseMethods(parsedABI, result.EVM.MethodIdentifiers)
	if err != nil {
	...
	// Parse events
	events, err := parseEvents(parsedABI)
	if err != nil {
...
func mapSolidityToGoType(abiType abi.Type) (types.GoType, error) {
	...
	case abi.TupleTy:
		// Tuples become named structs - this is handled at a higher level
		return types.GoType{
			TypeName: "struct{}", // placeholder
		}, nil
